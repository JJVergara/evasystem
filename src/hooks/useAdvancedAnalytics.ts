...
async function fetchSummaryMetrics(organizationId: string) {
...
async function fetchSocialMentionsDistribution(organizationId: string) {
  const { data: socialMentions } = await supabase
    .from('social_mentions')
    .select('mention_type')
    .eq('organization_id', organizationId);
...
async function fetchPerformanceDistribution(organizationId: string) {
  const { data: ambassadors } = await supabase
    .from('embassadors')
...
async function fetchEventComparison(selectedFiestaId: string | null | undefined) {
...
async function fetchTopAmbassadors(organizationId: string) {
  const { data: ambassadors } = await supabase
    .from('embassadors')
...
async function fetchTrends(organizationId: string) {
...
async function fetchAnalyticsData(
  ...
  const [summaryData, socialMentionsData, performanceData, ambassadorsData, trendsData] = await Promise.all([
    fetchSummaryMetrics(organizationId),
    fetchSocialMentionsDistribution(organizationId),
    fetchPerformanceDistribution(organizationId),
    fetchTopAmbassadors(organizationId),
    fetchTrends(organizationId)
  ]);

  // Fetch event comparison - only if fiesta is selected
  const eventData = selectedFiestaId ? await fetchEventComparison(selectedFiestaId) : [];
...
export function useAdvancedAnalytics(selectedFiestaId?: string | null, selectedEvent?: string, selectedPeriod?: string) {
  ...
  const queryKey = ['advancedAnalytics', organization?.id, selectedFiestaId, selectedEvent, selectedPeriod];

  const { data: analyticsData, isLoading: analyticsLoading, error } = useQuery({
    queryKey,
    queryFn: () => fetchAnalyticsData(organization!.id, selectedFiestaId),
  ...
  const loading = orgLoading || (!!organization?.id && analyticsLoading);

  return {
    analyticsData: analyticsData || null,
    loading,
    error: error ? 'Error al cargar datos analíticos' : null,
    refreshAnalytics
  };
}
