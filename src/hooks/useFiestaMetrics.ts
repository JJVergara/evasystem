...
function processTasksToMetrics(tasks: TaskWithRelations[]): FiestaMetricsData {
  ...
  return {
    ...
    recent_tasks:
      tasks?.slice(0, 10).map((task) => ({
        ...
        ambassador_name:
          `${task.embassadors?.first_name || ''} ${task.embassadors?.last_name || ''}`.trim() ||
          'Unknown',
...
async function fetchMetrics(fiestaId: string | null): Promise<FiestaMetricsData> {
  ...
  return fetchFiestaSpecificMetrics(fiestaId);
}
